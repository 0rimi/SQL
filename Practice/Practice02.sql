...
select  trunc(avg(salary),0),
        max(salary), min(salary), department_id
from employees
...
--예) 2001-01-13 토요일
select to_char(min(hire_date),'yyyy-mm-dd day') as "장기근속사원"
from employees;
...
select department_id,
        trunc(avg(salary),0),
        min(salary),
        trunc(avg(salary),0)-min(salary) as "평균임금-최저임금"
from employees
group by department_id
...
select  trunc(avg(salary),0) as "평균급여",
